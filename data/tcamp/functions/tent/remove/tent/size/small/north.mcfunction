...
execute if predicate tcamp:intact/tents/small/north run tag @s add tcamp.marker.campinfo.intact
...
#remove bedroll
execute as @s[tag=tcamp.marker.campinfo.intact] positioned ~0.5 ~ ~0.5 run function pbroll_camp:remove/roll/south

#replace structure with air
execute as @s[tag=tcamp.marker.campinfo.intact] run setblock ~ ~ ~ structure_block[mode=load]{name:"tcamp:tent_empty/small/tent_small_north",posX:-1,posY:0,posZ:-2,rotation:"NONE",mirror:"NONE",mode:"LOAD",ignoreEntities:1b} replace
...
execute as @s[tag=tcamp.marker.campinfo.intact] align xyz positioned ~ ~1 ~ run function tcamp:interactables/remove/confirmation/north
