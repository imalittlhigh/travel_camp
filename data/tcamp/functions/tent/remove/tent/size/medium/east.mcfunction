...
#remove bedroll
execute as @s[tag=tcamp.marker.campinfo.intact] positioned ~1.5 ~ ~-0.5 run function pbroll_camp:remove/roll/west

#replace structure with air
execute as @s[tag=tcamp.marker.campinfo.intact] run setblock ~ ~ ~ structure_block[mode=load]{name:"tcamp:tent_empty/medium/tent_medium_east",posX:-3,posY:0,posZ:-3,rotation:"NONE",mirror:"NONE",mode:"LOAD",ignoreEntities:1b} replace
...
execute as @s[tag=tcamp.marker.campinfo.intact] align xyz positioned ~ ~1 ~-1 run function tcamp:interactables/remove/confirmation/east
