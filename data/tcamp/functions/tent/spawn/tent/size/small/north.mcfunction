...
#setting structure
setblock ~ ~ ~ structure_block[mode=load]{name:"tcamp:tent/small/tent_small_north",posX:-1,posY:0,posZ:-2,rotation:"NONE",mirror:"NONE",mode:"LOAD",ignoreEntities:1b} replace
...
execute align xyz positioned ~ ~1 ~ run function tcamp:interactables/spawn/options/north
...
#set colour function
execute align xyz positioned ~0.5 ~ ~0.5 if data entity @e[tag=tcamp.marker.campinfo,limit=1,sort=nearest,distance=..1] data.camp{colour:"minecraft:red_dye"} run function tcamp:tent/modify/blocks/colour/size/small/north
