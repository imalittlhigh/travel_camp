...
execute if score %ccraft.count.0 ccraft_dummy matches 1.. if score %ccraft.count.1 ccraft_dummy matches 1.. if score %ccraft.count.2 ccraft_dummy matches 1.. if score %ccraft.count.3 ccraft_dummy matches 2.. if score %ccraft.count.4 ccraft_dummy matches 1.. if score %ccraft.count.5 ccraft_dummy matches 2.. if score %ccraft.count.6 ccraft_dummy matches 8.. if score %ccraft.count.7 ccraft_dummy matches 1.. if score %ccraft.count.8 ccraft_dummy matches 8.. run data modify storage ccraft:crafting success set value true
execute if data storage ccraft:crafting {success:true} run loot spawn ~ ~1 ~ loot ccraft:recipes/base_tent

##Remove number of items that should be used for crafting per slot
#                    Slot number 0-8 V           V number of items needed for caraft
scoreboard players remove %ccraft.count.0 ccraft_dummy 1
scoreboard players remove %ccraft.count.1 ccraft_dummy 1
scoreboard players remove %ccraft.count.2 ccraft_dummy 1
scoreboard players remove %ccraft.count.3 ccraft_dummy 2
scoreboard players remove %ccraft.count.4 ccraft_dummy 1
scoreboard players remove %ccraft.count.5 ccraft_dummy 2
